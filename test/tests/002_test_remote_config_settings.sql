...
BEGIN
  ...
  RETURN NEXT throws_ok('SELECT plproxy.set_remote_config('
   ...
   'insert or update on table "remote_config_settings" '
     ...
     || '"remote_config_settings_is_cached_parent_cluster_fkey"',
  ...
  RETURN NEXT throws_ok('UPDATE plproxy.remote_config_settings '
                        || 'SET is_cached = TRUE',
    '23514',
    'new row for relation "remote_config_settings" violates check constraint '
      || '"remote_config_settings_is_cached_check"',
    'throws error when attempting to set '
      || 'plproxy.remote_config_settings.is_cached to TRUE');

  RETURN NEXT throws_ok('UPDATE plproxy.clusters '
                        || 'SET is_cached = TRUE '
                        || 'WHERE cluster = ' || quote_literal(v_parent_cluster),
    '23503',
    'update or delete on table "clusters" violates foreign key constraint '
      || '"remote_config_settings_is_cached_parent_cluster_fkey" on table '
      || '"remote_config_settings"',
    'throws error when attempting to set '
      || 'plproxy.clusters.is_cached to TRUE for parent cluster');
